...
- `project.config.json` 已写入真实微信小程序 AppID。
- `miniprogram/app.js` 的 `webBaseUrl` 已替换成 HTTPS H5 业务域名。
- 微信公众平台已配置同一业务域名，否则 `web-view` 真机不会加载。

当前仓库默认保留本地开发地址 `http://127.0.0.1:5173/`，用于和 H5/安卓主线联调；这个状态不能直接上传正式版。
