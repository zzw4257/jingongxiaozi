...
function toolResultForRequest(request, mapState = {}, progress) {
  ...
  const activeIndex = Number(progress?.activeLegIndex || 0);
  ...
  const distanceMeters = Number(progress?.distanceMeters ?? progress?.next?.distanceMeters ?? 0);
  const remainingMeters = Number(progress?.remainingMeters ?? 0);
  if (request.tool === "navigation.next") {
    return progress
      ? { summary: `已切到第${activeIndex + 1}段，下一处是${checkpointLabel}`, visibleResult: `当前段：${instruction}` }
      : { summary: "已请求切到下一段", visibleResult: "小程序地图将切到下一段导航。" };
  }
  if (request.tool === "navigation.previous") {
    return progress
      ? { summary: `已回到第${activeIndex + 1}段，下一处是${checkpointLabel}`, visibleResult: `当前段：${instruction}` }
      : { summary: "已请求回到上一段", visibleResult: "小程序地图将回到上一段导航。" };
  }
  if (request.tool === "navigation.status") {
    return progress
      ? { summary: `当前第${activeIndex + 1}段，到${checkpointLabel}还有${Math.round(distanceMeters)}米`, visibleResult: `剩余约${Math.round(remainingMeters)}米：${instruction}` }
    ...
    return new Promise((resolve, reject) => {
      ...
      wx.request({
        ...
        success(response) {
          ...
          const required = ["map.open", "navigation.start", "navigation.next", "navigation.previous", "navigation.status"];
