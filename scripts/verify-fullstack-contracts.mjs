...
gitDuplex("fetch", "--quiet", "origin", "main");
const duplexUpstream = gitDuplex("rev-parse", "origin/main");
...
if (backendLock.commit !== duplexUpstream) {
  fail(`DuplexKit lock is not origin/main: lock=${backendLock.commit} upstream=${duplexUpstream}`);
}
