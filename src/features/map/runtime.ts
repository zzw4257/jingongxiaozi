...
export function navigationTtsPrompt(snapshot: Pick<NavigationProgressSnapshot, "activeLegIndex" | "current" | "next" | "remainingLegs" | "remainingMeters" | "isFinalLeg" | "guidance">): string {
...
export function buildNavigationProgressSnapshot(
  route: RouteResult,
  progress: RouteProgressState | undefined,
  source: RouteProgressState["source"] = progress?.source ?? "manual",
  heading?: NavigationProgressSnapshot["heading"],
): NavigationProgressSnapshot {
  const activeLegIndex = Math.min(
    Math.max(0, progress?.routeId === route.id ? progress.activeLegIndex : 0),
    Math.max(0, route.guidanceLegs.length - 1),
  );
  ...
  const phase: NavigationProgressSnapshot["guidance"]["phase"] =
    isFinalLeg ? "arrive" : leg.checkpointKind === "stair" ? "transition" : activeLegIndex === 0 ? "depart" : "walk";
  const currentSegmentLabel = `${leg.fromLabel} → ${leg.checkpointLabel}`;
  const nextActionLabel = isFinalLeg ? "到达终点后确认完成" : "到达该节点后点下一步，或说下一步";
  const snapshot: NavigationProgressSnapshot = {
